-- Expose tcgcsv per-subtype prices on the dashboard view and add jsonb storage.
-- (The tcgcsv catalog reset migration recreated the view without tcgplayer columns.)

begin;

alter table public.pokemon_card_images
  add column if not exists tcgplayer_prices_by_finish jsonb;

comment on column public.pokemon_card_images.tcgplayer_prices_by_finish is
  'tcgcsv /prices by subTypeName. Keys = tcgcsv subTypeName; values: subtype, market_cents, low_cents, high_cents, direct_cents.';

alter table public.pokemon_card_images
  add column if not exists tcgplayer_price_cents int;

comment on column public.pokemon_card_images.tcgplayer_price_cents is
  'Primary tcgcsv catalog price in USD cents (legacy aggregate / Normal heuristic).';
...
comment on view public.pokemon_card_images_with_market_activity is
  'pokemon_card_images + market activity, price delta, tcgplayer columns, card_number_sort_*, last_sold_comp_at.';
